...
recruiterApp.get("/getdetails/:id", expressAsyncHandler(async (req, res) => {
  try {
      const recruiterId = req.params.id;

      // Find recruiter by ID in the database
      ...
      // Send recruiter details (excluding password)
      res.status(200).json({
          id: recruiter._id,
          username: recruiter.username,
          email: recruiter.email,
      ...
      res.status(500).json({ message: "Error fetching recruiter details" });
  }
}));

//Update recruiter details
recruiterApp.put("/updateprofile", expressAsyncHandler(async (req, res) => {
  try {
    const { recruiterId, username, email, companyName, companyImageUrl } = req.body;

    // Validate the incoming data
    if (!recruiterId || !username || !companyName) {
      return res.status(400).json({ message: "All fields are required." });
    }

    // Find recruiter by ID and update their details
    const recruiter = await Recruiter.findById(recruiterId);
    if (!recruiter) {
      return res.status(404).json({ message: "Recruiter not found" });
    }
    ...
    recruiter.email = email;  // email might be updated or kept the same
    recruiter.companyName = companyName;
    recruiter.companyImageUrl = companyImageUrl || recruiter.companyImageUrl;  // If no new image URL, keep the old one

    // Save the updated recruiter details
    await recruiter.save();

    // Send back the updated recruiter data (excluding password)
    res.status(200).json({
    ...
    console.error(error);
    res.status(500).json({ message: "Error updating recruiter details" });
  }
}));
