...
userApp.post("/login", expressAsyncHandler(async (request, response) => {
  ...
  try {
      ...
      // Determine userType based on isStudent field
      const userType = user.isStudent ? "Student" : "JobSeeker";
      ...
      response.status(200).send({ message: "success", token: signedJWTToken, user: userResponse });
  } catch (error) {
...
userApp.get("/user-details/:userId", expressAsyncHandler(async (request, response) => {
  try {
    ...
    // Find the user by userId and populate fields if needed
    const user = await User.findById(userId).select('username email phoneNumber address skills isStudent resumeUrl studentDetails educationDetails');
...
userApp.get("/appliedjobs/:userId", expressAsyncHandler(async (request, response) => {
  try {
    ...
    for (const appliedJob of appliedJobs) {
      ...
      if (!jobDetails) {
        // console.error(`Job with ID ${appliedJob.jobId} not found`);
        continue; // Skip this iteration if job not found
      }
...
userApp.put("/updateProfile/:userId", expressAsyncHandler(async (request, response) => {
  try {
    const { userId } = request.params;
    const { username, email, phoneNumber, address, skills, isStudent, resumeUrl, studentDetails, educationDetails } = request.body;
    ...
    const updateFields = {
      ...
      isStudent,
      resumeUrl,
      ...(isStudent ? { studentDetails } : { educationDetails }),
    };
